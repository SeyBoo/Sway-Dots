...
wallpaper_current="$HOME/.config/sway/wallpaper_effects/.wallpaper_current"
...
# Monitor details (sway provides scale and current_mode dimensions)
scale_factor=$(swaymsg -t get_outputs | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .scale')
monitor_height=$(swaymsg -t get_outputs | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .current_mode.height')
...
modify_startup_config() {
  ...
  local startup_config="$HOME/.config/sway/UserConfigs/Startup_Apps.conf"

  # Check if it's a live wallpaper (video)
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    # For video wallpapers:
    sed -i '/^\s*exec\s*swww-daemon\s*--format\s*xrgb\s*$/s/^/\#/' "$startup_config"
    sed -i '/^\s*#\s*exec\s*mpvpaper\s*.*$/s/^#\s*//;' "$startup_config"
    ...
    sed -i '/^\s*#\s*exec\s*swww-daemon\s*--format\s*xrgb\s*$/s/^\s*#\s*//;' "$startup_config"

    sed -i '/^\s*exec\s*mpvpaper\s*.*$/s/^/\#/' "$startup_config"
