...
# Wallust: derive colors from the current wallpaper and update templates (Sway edition)
...
get_focused_monitor() {
  if command -v jq >/dev/null 2>&1; then
    ...
    swaymsg -t get_outputs | awk '/"focused": true/{found=1} found && /"name":/{gsub(/.*"name": "|",?$/, ""); print; exit}'
  fi
}
