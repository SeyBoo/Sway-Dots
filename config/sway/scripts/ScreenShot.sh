...
# Screenshots scripts (Sway edition)
...
# Get active window class via swaymsg
active_window_class=$(swaymsg -t get_tree | jq -r '.. | select(.focused? == true) | .app_id // .window_properties.class // "unknown"' 2>/dev/null | head -n1)
...
shotwin() {
	...
	w_geom=$(swaymsg -t get_tree | jq -r '.. | select(.focused? == true) | .rect | "\(.x),\(.y) \(.width)x\(.height)"' 2>/dev/null | head -n1)
...
shotactive() {
    # Re-fetch active window info at shot time
    active_window_class=$(swaymsg -t get_tree | jq -r '.. | select(.focused? == true) | .app_id // .window_properties.class // "unknown"' 2>/dev/null | head -n1)
    active_window_file="Screenshot_${time}_${active_window_class}.png"
    active_window_path="${dir}/${active_window_file}"

    # Get geometry of focused window
    swaymsg -t get_tree | jq -r '.. | select(.focused? == true) | .rect | "\(.x),\(.y) \(.width)x\(.height)"' 2>/dev/null | head -n1 | grim -g - "${active_window_path}"
	sleep 1
    notify_view "active"
}
