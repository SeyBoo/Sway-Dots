...
# Local Paths
local_dir="$HOME/.config/sway"
iDIR="$HOME/.config/swaync/images/"
local_version=$(find "$local_dir" -maxdepth 1 -name 'v*' -printf '%f\n' 2>/dev/null | sort -V | tail -n 1 | sed 's/^v//')
KooL_Dots_DIR="$HOME/Sway-Dots"

# exit if cannot find local version
if [ -z "$local_version" ]; then
  notify-send -i "$iDIR/error.png" 'ERROR !?!?!!' "Unable to find KooL's Sway-Dots version. Exiting."
  exit 1
fi

# GitHub URL - KooL's Sway-Dots
branch="main"
github_url="https://github.com/seyboo/Sway-Dots/tree/$branch/config/sway/"
...
# Fetch the version from GitHub URL - KooL's Sway-Dots
github_version=$(curl -fsSL -A "Mozilla/5.0" "$github_url" | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' | sort -V | tail -n 1 | sed 's/v//')

# Cant find GitHub URL - KooL's Sway-Dots version
if [ -z "$github_version" ]; then
  notify-send -i "$iDIR/error.png" 'KooL Sway-Dots:' "Unable to determine GitHub version."
  exit 1
fi

# Comparing local and github versions
if [ "$(echo -e "$github_version\n$local_version" | sort -V | head -n 1)" = "$github_version" ]; then
   notify-send -i "$iDIR/note.png" "KooL Sway-Dots:" "No update available"
  exit 0
else
  # update available
  ...
  response=$($notify_cmd_shot "KooL Sway-Dots:" "Update available! Update now?")
  ...
  case "$response" in
    "action1")
      if [ -d "$KooL_Dots_DIR" ]; then
        ...
        kitty -e bash -c "
          git clone --depth=1 https://github.com/seyboo/Sway-Dots.git $KooL_Dots_DIR &&
          cd \"$KooL_Dots_DIR\" &&
          chmod +x copy.sh &&
          ./copy.sh &&
		  notify-send -u critical -i \"$iDIR/ja.png\" 'Update Completed:' 'Kindly log out and relogin to take effect'
