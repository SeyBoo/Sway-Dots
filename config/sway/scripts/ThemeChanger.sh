...
# Sway edition: swaymsg reload used instead of compositor reload; Refresh.sh path updated
...
if wallust theme -- "${choice}"; then
  ...
  targets=(
    ...
    "$HOME/.config/sway/wallust/wallust-sway.conf"
  ...
  # Reload Sway so new border colors from wallust-sway.conf take effect
  swaymsg reload >/dev/null 2>&1 || true

  # Refresh bars/menus after files are ready
  if [ -x "$HOME/.config/sway/scripts/Refresh.sh" ]; then
    "$HOME/.config/sway/scripts/Refresh.sh" >/dev/null 2>&1 || true
  else
